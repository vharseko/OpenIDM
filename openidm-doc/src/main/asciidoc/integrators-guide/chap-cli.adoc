...
----
The `-a` or `--algorithm` option specifies the hash algorithm to use. OpenIDM supports the following hash algorithms: `SHA-256`, `SHA-384`, and `SHA-512`. If you do not specify a hash algorithm, `SHA-256` is used. Values hashed with `MD5` or `SHA-1` by earlier releases are still verified, but new hashes can no longer be created with those algorithms.
...
-----BEGIN HASHED VALUE-----
{
  "$crypto" : {
    "value" : {
      "algorithm" : "SHA-384",
      "data" : "fS1S4Dqbv3aHHL9f+Q7KEdnoqqYyrsLqHqFtTurfl2aWCxSO6MpbVm2WVSGMn/i8BQLcaAqwa1LFoQXTSS6zOw=="
    },
    "type" : "salted-hash"
  }
}
